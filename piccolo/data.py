...
class UniDataset(Dataset):
    ...
    def __getitem__(self, index: int):
        ...
        if hf_dataset[0]["type"] == "cls_contrast":
            ...
            "text" in hf_dataset[0] and "text_pos" in hf_dataset[0]
        ):  # pair contrast dataset
            pair_records = self.get_pair_contrast_records(
                records, task_name, hf_dataset, batch_index
            )
        elif (
            "text" in hf_dataset[0]
            and "text_pair" in hf_dataset[0]
            and "label" in hf_dataset[0]
        ):  # pair scored dataset
